...
double EnqueueKernel( cl_command_queue* queue, cl_kernel* kernel, cl_uint work_dim, const size_t* global_work_size, const size_t* local_work_size)
{
    cl_int err;
    cl_event event;

    // Enqueue kernel once before time measurement to exclude kernel setup overhead for OpenCL
    err = clEnqueueNDRangeKernel( *queue, *kernel, work_dim, NULL, global_work_size, local_work_size, 0, NULL, &event);
    ERR();
    err = clFinish( *queue );
    ERR();
...
size_t max_local_size ( cl_context* context, size_t global_size )
{
    cl_int err;
    size_t query_size, max_size;
    ...
    // Print the max work group size of first device
    clGetDeviceInfo(devices[0], CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t), &max_size, 0);
    std::cout << "Max work group size: " << max_size << std::endl;
    ...
    {
        for ( uint i = global_size>>1; i>0; i-- )
        {
            if ( global_size % i == 0 && i <= max_size )
                return i;
        }
        return 1;
    }
...
int main(int argc, char* argv[])
{
    ...
    std::string kernel_name = "float_dot_prod";
    typedef float Numeric_Type;
    size_t vector_size, global_work_size, local_work_size;
    ...
    local_work_size = max_local_size( &context, global_work_size );
    std::cout << "Selected work group size: " << local_work_size << std::endl;
    ...
    SetKernelArg( &kernel, 4, local_work_size*sizeof(cl_float), NULL );
    ...
    double ocl_time = EnqueueKernel( &queue, &kernel, 1, &global_work_size, &local_work_size);
    ...
    int flag = 0;
    for ( uint i = 0; i < global_work_size/local_work_size; i++ )
    {
